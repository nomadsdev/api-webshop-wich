...
const URL_API_VERIFY_TRUEMONEY = "https://truemoneyapi.vercel.app/api/truewallet";
...
router.post("/verify-gift", auth, async (c: AuthContext) => {
  try {
    ...
    // Call TrueMoney API to verify gift
    const verifyResponse = await axios.post(URL_API_VERIFY_TRUEMONEY, {
    ...
    const topupHistory = new TopupHistory({
      userId: user._id,
      transactionId: `TM_${Date.now()}`, // Generate unique transaction ID for TrueMoney
      amount: giftAmount,
      ...
      status: 'success',
      type: 'truemoney_gift' // Add type to distinguish from bank slips
    });
    ...
    if (error.response) {
      ...
      if (status === 400 && errorData.status === 'error') {
        // Handle specific TrueMoney API errors

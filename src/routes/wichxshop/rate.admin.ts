...
import { RateConfig, getProductRate, calculateProductPrice, initializeDefaultRates } from "./config.rate.js";

const router = new Hono();

// Get all rates (including global)
router.get("/rates", auth, authAdmin, async (c: AuthContext) => {
  try {
    const rates = await RateConfig.find({}).sort({ isGlobal: -1, createdAt: -1 });
...
// Get specific rate by productId
...
// Create or update rate (supports global rates)
router.post("/rates", auth, authAdmin, async (c: AuthContext) => {
  try {
    const body = await c.req.json();
    const { productId, percentage, fixedAmount, customPrice, isActive, description, isGlobal, rateMultiplier } = body;
    ...
    // ✅ New validation for percentage and fixedAmount
    ...
    // ✅ Backward compatibility: allow rateMultiplier but convert it
    let convertedPercentage = percentage;
    let convertedFixedAmount = fixedAmount;
    
    if (rateMultiplier !== undefined && percentage === undefined && fixedAmount === undefined) {
    ...
    const updateData: any = {
      ...(productId && { productId }),
      ...(convertedPercentage !== undefined && { percentage: convertedPercentage }),
      ...(convertedFixedAmount !== undefined && { fixedAmount: convertedFixedAmount }),
      ...(customPrice !== undefined && { customPrice }),
      ...(isActive !== undefined && { isActive }),
      ...(description !== undefined && { description }),
      isGlobal: isGlobal || false,
      // Clear deprecated rateMultiplier field
      rateMultiplier: null
    };
    
    // For global rates, use a unique identifier
    const query = isGlobal ? { isGlobal: true } : { productId };
    
    const rateConfig = await RateConfig.findOneAndUpdate(
      query,
      updateData,
    ...
    return c.json({
      success: true,
      message: isGlobal ? "บันทึกอัตราส่วนกลางสำเร็จ" : "บันทึกการตั้งค่าค่าธรรมเนียมสำเร็จ",
      data: {
        id: rateConfig._id,
        productId: rateConfig.productId,
        percentage: rateConfig.percentage,
...
router.post("/rates/bulk", auth, authAdmin, async (c: AuthContext) => {
  try {
    ...
    for (const rateData of rates) {
      const { productId, percentage, fixedAmount, customPrice, isActive, description, rateMultiplier } = rateData;
      
      if (!productId) continue;
      
      // ✅ Backward compatibility: convert rateMultiplier to percentage
      let convertedPercentage = percentage;
      let convertedFixedAmount = fixedAmount;
...
router.post("/calculate-price", async (c) => {
  try {
    ...
    const finalPrice = await calculateProductPrice(Number(originalPrice), productId);
    
    // Get both specific and global configs for display
    ...
    return c.json({
      ...
      data: {
        ...
        percentage: originalPrice > 0 ? Math.round(((originalPrice - finalPrice) / originalPrice) * 100) : 0,
        calculationType: "% + Fixed" // ✅ Updated calculation label
      }
    });
  } catch (error: any) {

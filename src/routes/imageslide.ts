...
router.get("/", async (c) => {
  try {
    ...
    const slides = await ImageSlide.find({ isActive: true })
...
router.post("/admin", auth, authAdmin, async (c: AuthContext) => {
  try {
    ...
    const nextOrder = order !== undefined ? order : (maxOrderSlide?.order || 0) + 1;
...
router.put("/admin/reorder", auth, authAdmin, async (c: AuthContext) => {
  try {
    ...
    const updatePromises = slides.map(({ id, order }: { id: string; order: number }) =>
